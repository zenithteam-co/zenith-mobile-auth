...
class Zenith_Mobile_Auth_Ajax {
    ...
    public function send_otp() {
        ...
        if ( $sent === true ) {
            ...
            wp_send_json_success( [ 'message' => __('Code Sent', 'zenith-mobile-auth'), 'phone' => $phone, 'token' => $otp_token ] );
        } else {
    ...
    public function update_user_info() {
        ...
        $session_data = get_transient( 'zma_temp_session_' . $session_token );

        if ( ! $session_data || ! isset($session_data['user_id']) ) {
            wp_send_json_error( [ 'message' => __('Session expired. Please restart login.', 'zenith-mobile-auth') ], 403 );
        }
        ...
        wp_set_auth_cookie( $user_id, true );
        wp_set_current_user( $user_id );
        delete_transient( 'zma_temp_session_' . $session_token );

        $redirect = isset($session_data['redirect_to']) ? $session_data['redirect_to'] : home_url();
        wp_send_json_success( [ 'message' => __('Profile Updated. Redirecting...', 'zenith-mobile-auth'), 'redirect_to' => $redirect ] );
    }
